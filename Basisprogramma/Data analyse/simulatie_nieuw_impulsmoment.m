function [m,M,r,x,v,ap,beginM,beginr,bpm,impulsmoment] = simulatie_nieuw_impulsmoment(p,dt,T,minR,maxR,minM,maxM,dat,filenaam)
    ...
    impulsmoment = zeros(ceil(T/(12*dat)),1); %impulsmoment op bepaalde meetmomenten
    ...
    for k = 3:T
        ...
        if mod(k,12*dat) == 0 % een meting elke dat jaar
            ...
            ap(k2) = sum(isplaneet);
            %bereken impulsmoment van systeem
            vopmoment = v(:,k,:)+a*dt/2; %eulervw om v op k*dt te benaderen
            IMP = 0;
            for u=1:length(m)
                plaats = (x(u,k,1),x(u,k,2),0);
                snelheid = (vopmoment(u,1)+vopmoment(u,2),0);
                IMP = IMP+cross(plaats,m(u)*snelheid);
            end
            impulsmoment(k2) = IMP;
            k2 = k2+1;
