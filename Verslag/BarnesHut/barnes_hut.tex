...
\textbf{In conclusion, we can not state the Barnes-Hut algorithm gives any reduction on the number of calculations, but on average, the number of calculations reduces from $\orde{N^2}$ to $\orde{N\log(N)}$.} 
