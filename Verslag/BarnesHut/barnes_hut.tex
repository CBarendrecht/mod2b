...
\item Note that the calculated force is always less or equal the actual force if the objects are aligned with respect to the object $a$ the force is calculated on. However if the objects are next to each other with respect to $a$ and have the same distance to $a$, the calculated force is greater due to the "head-tail" method. In general, the calculated force is less than the actual force most of the time.
